...
Elm.Hangman.make = function (_elm) {
   ...
   var Guessing = {ctor: "Guessing"};
   var status = function (model) {    return winner(model) ? Winner : _U.cmp(wrongAttempts(model),10) < 0 ? Guessing : Loser;};
   var message = function (model) {
      var _p4 = status(model);
      switch (_p4.ctor)
      {case "Guessing": return $Html.text(A2($Basics._op["++"],"Attempts remaining ",$Basics.toString(10 - wrongAttempts(model))));
   ...
   var update = F2(function (action,model) {
      ...
      {case "Guess": return {ctor: "_Tuple2"
                            ,_0: _U.eq(status(model),Guessing) ? _U.update(model,{used: A2($Set.insert,$Char.toLower(_p5._0),model.used)}) : model
                            ,_1: $Effects.none};
         case "NewWord": return {ctor: "_Tuple2",_0: {word: A2($Maybe.withDefault,model.word,A2($Debug.log,"new",_p5._0)),used: $Set.empty},_1: $Effects.none};
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,_U.list([$Html$Attributes.$class("splash-head")]),_U.list([$Html.text($String.toUpper(A2(showSelected,model.word,model.used)))]))
   ...
   return _elm.Hangman.values = {_op: _op
                                ,Model: Model
                                ,init: init
                                ,Guess: Guess
