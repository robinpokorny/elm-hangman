...
Elm.Hangman = Elm.Hangman || {};
Elm.Hangman.make = function (_elm) {
   "use strict";
   _elm.Hangman = _elm.Hangman || {};
   if (_elm.Hangman.values) return _elm.Hangman.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var wotdUrl = A2($Http.url,
   "http://api.wordnik.com:80/v4/words.json/randomWord",
   _U.list([{ctor: "_Tuple2",_0: "hasDictionaryDef",_1: "true"}
           ,{ctor: "_Tuple2",_0: "includePartOfSpeech",_1: "noun"}
           ,{ctor: "_Tuple2",_0: "minCorpusCount",_1: "100000"}
           ,{ctor: "_Tuple2",_0: "maxCorpusCount",_1: "-1"}
           ,{ctor: "_Tuple2",_0: "minDictionaryCount",_1: "1"}
           ,{ctor: "_Tuple2",_0: "maxDictionaryCount",_1: "-1"}
           ,{ctor: "_Tuple2",_0: "minLength",_1: "5"}
           ,{ctor: "_Tuple2",_0: "maxLength",_1: "-1"}
           ,{ctor: "_Tuple2",_0: "api_key",_1: "8df7629a24338772eb60b0335210c7bd7b795e938e9075b8b"}]));
   ...
   var onKeyUp = F2(function (address,handler) {
      return A3($Html$Events.on,"keyup",$Html$Events.keyCode,function (code) {    return A2($Signal.message,address,handler($Char.fromCode(code)));});
   });
   ...
   var Loser = {ctor: "Loser"};
   var Winner = {ctor: "Winner"};
   var Guessing = function (a) {    return {ctor: "Guessing",_0: a};};
   var decodeJson = A2($Json$Decode.at,_U.list(["word"]),$Json$Decode.string);
   var NewWord = function (a) {    return {ctor: "NewWord",_0: a};};
   var getRandomWord = $Effects.task(A2($Task.map,NewWord,$Task.toMaybe(A2($Http.get,decodeJson,wotdUrl))));
   var RequestMore = {ctor: "RequestMore"};
   var Guess = function (a) {    return {ctor: "Guess",_0: a};};
   var inputField = function (address) {
      return A2($Html.input,_U.list([A2(onKeyUp,address,Guess),$Html$Attributes.value(""),$Html$Attributes.$class("splash-input")]),_U.list([]));
   };
   var attemptsCount = 6;
   var status = function (model) {
      if (winner(model)) return Winner; else {
            var wrong = wrongAttempts(model);
            return _U.cmp(wrong,attemptsCount) < 0 ? Guessing(attemptsCount - wrong) : Loser;
         }
   };
   var update = F2(function (action,model) {
      var _p4 = action;
      switch (_p4.ctor)
      {case "Guess": return {ctor: "_Tuple2"
                            ,_0: function () {
                               var _p5 = status(model);
                               if (_p5.ctor === "Guessing") {
                                     return _U.update(model,{used: A2($Set.insert,$Char.toLower(_p4._0),model.used)});
                                  } else {
                                     return model;
                                  }
                            }()
                            ,_1: $Effects.none};
         case "NewWord": return {ctor: "_Tuple2",_0: {word: A2($Maybe.withDefault,model.word,_p4._0),used: $Set.empty},_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: getRandomWord};}
   });
   var message = function (model) {
      var _p6 = status(model);
      switch (_p6.ctor)
      {case "Guessing": return $Html.text(A2($Basics._op["++"],"Attempts remaining ",$Basics.toString(_p6._0)));
         case "Winner": return $Html.text("Winner!");
         default: return A2($Html.span,
           _U.list([]),
           _U.list([A2($Html.strong,_U.list([]),_U.list([$Html.text("Loser! ")]))
                   ,$Html.text("Word was: ")
                   ,A2($Html.em,_U.list([]),_U.list([$Html.text(model.word)]))]));}
   };
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("splash")]),
      _U.list([A2($Html.h1,
              _U.list([$Html$Attributes.$class("splash-head")]),
              _U.list([inputField(address),$Html.text($String.toUpper(A2(showSelected,model.word,model.used)))]))
              ,A2($Html.p,_U.list([$Html$Attributes.$class("splash-subhead")]),_U.list([message(model)]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,RequestMore),$Html$Attributes.href("#"),$Html$Attributes.$class("pure-button pure-button-primary")]),
              _U.list([$Html.text("New word")]))]))]));
   });
   var init = {ctor: "_Tuple2",_0: {word: "hangman",used: $Set.empty},_1: getRandomWord};
   var Model = F2(function (a,b) {    return {word: a,used: b};});
   return _elm.Hangman.values = {_op: _op,init: init,view: view,update: update,Model: Model,RequestMore: RequestMore,Guess: Guess};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var codeToAction = function (c) {    return $Hangman.Guess($Char.fromCode(c));};
   var guessChar = A2($Signal.map,codeToAction,$Keyboard.presses);
   var app = $StartApp.start({init: $Hangman.init,view: $Hangman.view,update: $Hangman.update,inputs: _U.list([guessChar])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.Main.values = {_op: _op,app: app,guessChar: guessChar,codeToAction: codeToAction,main: main};
};
